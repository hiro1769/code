...
class PointNetSetAbstractionMsg(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        for i, radius in enumerate(self.radius_list):
            ...
            # print(f"group_idx shape: {group_idx.shape}")
            # print(f"max group_idx: {torch.max(group_idx)}, min group_idx: {torch.min(group_idx)}")
            # print(f"N: {N}")
    
            # 确保 group_idx 中的索引值在 xyz 的范围内
            assert torch.max(group_idx) < N, "Index out of range"
            assert torch.min(group_idx) >= 0, "Index out of range"
